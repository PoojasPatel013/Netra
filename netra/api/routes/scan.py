                    ...
                    if "server" in data:
                        session.add(
                            Vulnerability(
                                ...
                                type="Tech Stack",
                                severity="Info",
                                description=f"Server: {data['server']}",
                            )
                        )
...
async def get_results(target: str, session: Session = Depends(get_session)):
    ...
    statement = (
        select(Scan).where(Scan.target == target).order_by(Scan.timestamp.desc())
    )
    ...
    for v in vulns:
        if v.type == "Open Port":
            ...
            formatted_results["HTTPScanner"]["server"] = v.description.replace(
                "Server: ", ""
            )
        else:
