...
class ZombieScanner(BaseScanner):
    ...
    async def _find_shadow_apis(
        self, base_url: str, client: SafeHTTPClient, results: Dict, schema: Dict = None
    ):
        import re
    ...
    async def _fuzz_schema(
        self, schema: Dict, base_url: str, client: SafeHTTPClient, results: Dict
    ):
            ...
            for method_name, details in methods.items():
                ...
                fuzz_tasks.append(
                    self._check_endpoint(
                        client, method_name, url, "BOLA/ID Check", results
                    )
                )
                ...
                if err_path != fuzzed_path:
                    ...
                    fuzz_tasks.append(
                        self._check_endpoint(
                            client, method_name, err_url, "Error Handling", results
                        )
                    )

                results["endpoints_fuzzed"] += 1
                if (
                    results["endpoints_fuzzed"] > 20
                ):  # Cap at 20 endpoints to avoid excessive traffic
                    break
