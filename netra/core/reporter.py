...
class SARIFReporter:
            ...
            self.rules.append(
                {
                    ...
                    "properties": {"tags": ["security", "netra"]},
                }
            )
    ...
    def add_result(
        ...
        rule_id = (
            f"{scanner_name.upper()}_{vulnerability_type.replace(' ', '_').upper()}"
        )
        self._get_or_create_rule(
            rule_id,
            vulnerability_type,
            f"{vulnerability_type} detected by {scanner_name}",
        )
    ...
    def convert_scan_results(
        self, scan_results: Dict[str, Any], target: str
    ) -> Dict[str, Any]:
        """
