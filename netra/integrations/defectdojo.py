...
class DefectDojoClient:
    ...
    async def import_scan(
        self, scan_results: dict, engagement_id: int, scan_type: str = "Netra Scan"
    ):
        """
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with session.post(endpoint, data=data) as resp:
                    if resp.status == 201:
                        logger.info(
                            f"Successfully imported scan to DefectDojo. ID: {engagement_id}"
                        )
                        return {"ok": True, "status": resp.status}
                    else:
                        resp_text = await resp.text()
                        logger.error(
                            f"Failed to import to DefectDojo: {resp.status} - {resp_text}"
                        )
